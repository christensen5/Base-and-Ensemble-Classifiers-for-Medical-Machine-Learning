...
\abstract{\textbf{Motivation:} Rapid expansion of the amount of medical data available to the healthcare industry is driving demand for more accurate, precise and reliable tools for exploiting these data. Real-world experience suggests that ensemble methods for machine learning might be able to meet all three criteria at a low cost, potentially saving scarce funds and patients' lives. We attempt to go beyond the existing literature on individual machine learning algorithms to discover the effectiveness of combining their predictions, and to explore the suitability of such so-called 'ensemble methods' to personalised diagnostic and prognostic medicine.\\
\\
\textbf{Results:} 
Consideration of a range of classifiers is an important aspect in getting the best predictive performance for a given data set.  For the range of 10 biomedical data sets considered in this paper, a range of classifier ensemble methods gave competitive performance and it was possible to reliably automate these, making them straightforward to apply to new data sets.  It was noteworthy that unsupervised methods such as averaging of prediction probabilities generally performed well, which is an advantage as no separate training examples are required for a meta-learning step.  Most strikingly, using the meta-learning step to select the best-performing base classifier was actually a very strong strategy, leading to best or near-best predictive performance on all 10 data sets.
\\
We conclude that the widely observed predictive performance improvements seen in e.g. data science competitions may be more the result of averaging over different set of features and feature representations, rather than over different types of model.  We also conclude that there may be significant merit in the development of formal methods for selecting between a wide range of different classifiers.
\\
...
\bibitem[3]{classifiercomparison}
