...
##
CVSparseLogisticRegression <- function(data.train, targetValues, nFolds=10, threshold=NULL, nKeep=NULL, verbose=FALSE, heatmap=FALSE, PCA=FALSE, extraData=NULL){
  ##----------------------------------------------------------------------
  ## LOAD LIBRARIES ------------------------------------------------------
  ##----------------------------------------------------------------------
  ...
  for (i in 1:nFolds){
    ...
    cv.fit          = cv.glmnet(as.matrix(trainingData), trainingTargets, family="binomial", maxit=1e5, alpha=1)
  	working = predict(cv.fit, as.matrix(testData), type="response", s="lambda.min")
    ...
    if (i==1){
      nModels                = length(working)
      predictions            = matrix(0, nDataItems, 1)
    ...
    for(j in 1:nModels){
    		predictions[index[j], 1] = working[j]
    	}    
  }
  return(predictions)
}
